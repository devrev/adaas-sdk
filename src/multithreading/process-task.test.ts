...
describe(processExtractionTask.name, () => {
  ...
  it('should emit a progress result on timeout when onTimeout is omitted in a resumable phase', async () => {
    // Arrange
    ...
    // Act
    processExtractionTask({ task });
    await flush();

    // Assert
    expect(mockAdapter.emitFromResult).toHaveBeenCalledWith({
      status: 'progress',
    });
  });

  it('should emit a timeout error result when onTimeout is omitted in a non-resumable phase', async () => {
    // Arrange
    const event = makeEvent(EventType.StartExtractingMetadata);
    setWorkerData({ event, initialState: {}, options: {} });
    const mockAdapter = {
      event,
      isTimeout: true,
    ...
    expect(mockAdapter.emitFromResult).toHaveBeenCalledWith({
      status: 'error',
      error: {
        message: expect.stringContaining('non-resumable phase'),
      },
    });
  });
