...
/** Adapter passed to loading tasks (item/attachment loading, mappers, loader reports). @public */
...
> extends BaseAdapter<ConnectorState> {
  ...
  async loadAttachments({
    ...
    try {
      ...
      console.error('Error during attachment loading.', serializeError(error));
      return {
        status: 'error',
        error: {
          message: `Error during attachment loading. ${serializeError(error)}`,
        },
      };
    }
