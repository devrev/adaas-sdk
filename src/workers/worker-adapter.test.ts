...
import { EventType, AdapterState, ExtractorEventType } from '../types';
...
jest.mock('node:worker_threads', () => {
  const actualModule = jest.requireActual('node:worker_threads');
  return {
    ...actualModule,
    parentPort: {
      postMessage: jest.fn(),
...
describe(WorkerAdapter.name, () => {
  ...
  describe(WorkerAdapter.prototype.emit.name, () => {
    let mockParentPort: { postMessage: jest.Mock };

    beforeEach(() => {
      // Get the mocked parentPort from the mocked module
      const mockedWorkerThreads = jest.requireMock('node:worker_threads');
      mockParentPort = mockedWorkerThreads.parentPort;
      if (mockParentPort?.postMessage) {
        (mockParentPort.postMessage as jest.Mock).mockClear();
      }
    });
