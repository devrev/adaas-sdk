...
import {
  ExtractionSdkState,
  initialExtractionSdkState,
  StateInterface,
} from './state.interfaces';

/**
 * ExtractionState is the per-mode state for extraction workers. It narrows
 * `sdkState` to `ExtractionSdkState` (extraction boundaries + attachments
 * bookkeeping) on top of the shared lifecycle provided by `BaseState`.
 */
export class ExtractionState<ConnectorState> extends BaseState<
  ConnectorState,
  ExtractionSdkState
> {
  constructor(params: StateInterface<ConnectorState>) {
    super(params, initialExtractionSdkState);
  }
 ...
 *
 * For non-stateless events this fetches/migrates persisted state, installs the
 * initial domain mapping if the snap-in version changed, then resolves the
 * extraction window (time-value resolution + pending boundary reuse) and
 * validates it.
 */
